        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {active.map(manager => (
            <Link href={`/managers/${manager.manager}`} key={manager.manager} className="block">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-emerald-500/50 hover:bg-slate-800/50 transition-all cursor-pointer group h-full flex flex-col justify-center shadow-lg">
      ...
      {former.length > 0 && (
        <section className="space-y-8 pt-8 border-t border-slate-800/50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 opacity-60">
            {former.map(manager => (
              <Link href={`/managers/${manager.manager}`} key={manager.manager} className="block">
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 hover:opacity-100 hover:border-slate-700 transition-all cursor-pointer group h-full flex flex-col justify-center">
